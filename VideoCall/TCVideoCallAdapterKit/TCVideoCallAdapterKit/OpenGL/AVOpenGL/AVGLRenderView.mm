...
- (void)drawFrame
{
    ...
    //    glViewport(_frame.origin.x*scale,((GL_SCREEN_WIDTH?GL_SCREEN_HEIGHT:GL_SCREEN_WIDTH) - _frame.size.height - _frame.origin.y)*scale, width, height);
    
    //renqingyang
    //在客户端屏幕发生旋转时，GL_SCREEN_HEIGHT，GL_SCREEN_WIDTH不会发生变化。如需要改为竖屏，需要把注释打开.
//    glViewport(_frame.origin.x*scale,(GL_SCREEN_WIDTH - _frame.size.height - _frame.origin.y)*scale, width, height);
    glViewport(_frame.origin.x*scale,_frame.origin.y*scale, width, height);
